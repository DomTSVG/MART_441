// copy this to test prompt - cd desktop\documents\github\mart_441\hw-14 -
...
function readQueryString(queryObject) {
   ...
   if (queryObject != null) {
      ...
      if (food != undefined && color != undefined) {
         resp = "Your favorite food is " + food + " and your favorite color is " + color;
         // change the file
         updateFile(resp);
      }
...
function updateFile(resp)
{
    // open the html file
   fs.readFile('display.html', 'utf8', (err,html)=>{
      if(err){
         throw err;
      }
      ...
      mySpan.set_content(resp);

      // create the stream
      var writerStream = fs.createWriteStream("display.html");
